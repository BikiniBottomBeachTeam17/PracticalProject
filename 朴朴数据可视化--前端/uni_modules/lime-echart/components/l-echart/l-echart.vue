...
// #endif
// #ifndef APP-NVUE
import {Canvas, setCanvasCreator, dispatch} from '../../static/canvas';
import { compareVersion, wrapTouch, devicePixelRatio } from '../../static/utils';
// #endif
// #ifdef APP-NVUE
import { base64ToPath } from '../../static/utils';
// #endif
const charts = {}
const echartsObj = {}
export default {
	...
	methods: {
		...
		async init(echarts, callback) {
			...
			if(arguments.length < 2) {
				console.error('缺少参数：init(echarts, config => {})')
				return
			}
			...
			if(!arguments.length) {
				console.error('缺少参数：init(config => {})')
				return
			}
				...
				return new Promise(resolve => {
					uni.createSelectorQuery()
						...
						.exec(res => {
							...
							this.config = { canvas, width, height, devicePixelRatio: dpr, node };
							resolve(this.config);
						});
				});
			}
